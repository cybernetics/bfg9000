...
def package(context, name, version=None, *, lang=None,
            kind=PackageKind.any.name, headers=default_sentinel,
...
def boost_package(context, name=None, version=None):
    ...
    resolver = context.env.builder('c++').packages
    pkg = resolver.resolve('boost', name, version, PackageKind.any,
                           get_version=_boost_version)
